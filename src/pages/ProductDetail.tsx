  ...
  if (!product) {
    return (
      <div className="container py-32 text-center">
        ...
        <Link to="/shop" className="inline-block mt-6 underline">Back to shop</Link>
      </div>
    );
  }
  ...
  return (
    <>
      <div className="container pt-8">
        <Link to="/shop" className="inline-flex items-center gap-2 text-xs uppercase tracking-widest text-muted-foreground hover:text-foreground transition-colors">
      ...
      <section className="container py-10 md:py-14 grid lg:grid-cols-2 gap-12 lg:gap-20">
        ...
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 0.6 }}>
          ...
          <div className="mt-4 grid grid-cols-3 gap-3">
            {gallery.map((g, i) => (
              ...
              >
                <img src={g} alt="" width={300} height={300} className="w-full h-full object-cover" />
              </button>
            ))}
          </div>
        </motion.div>
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6, delay: 0.1 }}>
          ...
          <p className="mt-8 text-base leading-relaxed text-muted-foreground">{product.description}</p>

          <ul className="mt-6 space-y-2 text-sm">
            {["Hydrating, non-sticky formula", "Vitamin E enriched", "Cruelty-free & vegan", "Long-wear pigment"].map((f) => (
          ...
          <div className="mt-10 flex items-center gap-6">
            ...
            <div className="flex items-center border border-border rounded-full">
              <button onClick={() => setQty((q) => Math.max(1, q - 1))} className="w-10 h-10 flex items-center justify-center hover:text-primary">
                <Minus className="w-3.5 h-3.5" />
              </button>
              <span className="w-10 text-center text-sm">{qty}</span>
              <button onClick={() => setQty((q) => q + 1)} className="w-10 h-10 flex items-center justify-center hover:text-primary">
          ...
          <div className="mt-8 flex flex-col sm:flex-row gap-3">
            ...
            <button className="inline-flex items-center justify-center w-14 h-14 rounded-full border border-border hover:border-primary hover:text-primary transition-colors">
      ...
      {related.length > 0 && (
        <section className="container py-20">
          <h2 className="font-display text-3xl md:text-4xl text-center mb-10">You may also love</h2>
